...
from tensorflow.keras.layers import Dense, GRU, Input, Lambda, RepeatVector, TimeDistributed
...
def create_model():
    ...
    # GRU encoding
    h = GRU(units=INTERMEDIATE_DIM)(x)
    ...
    # decoded GRU layer
    decoder_h = GRU(INTERMEDIATE_DIM, return_sequences=True, return_state=True, input_shape=(None, INPUT_DIM))

    # todo: not sure if this initialization is correct
    h_decoded, _ = decoder_h(decoder_words_input, initial_state=[z_reweighted_h])
    ...
    _h_decoded, _decoded_h = decoder_h(decoder_words_input, initial_state=[_z_rewighted_h])
    _decoded_onehot = decoder_dense(_h_decoded)
    generator = Model([decoder_words_input, decoder_state_input], [_decoded_onehot, _decoded_h])

    # RNN for inference
    input_h = Input(shape=(INTERMEDIATE_DIM,))
    __h_decoded, __decoded_h = decoder_h(decoder_words_input, initial_state=[input_h])
    __decoded_onehot = decoder_dense(__h_decoded)
    stepper = Model([decoder_words_input, input_h], [__decoded_onehot, __decoded_h])
