...
class MoleculeVAE():
    ...
    def create(self,
        ...
        maskv = Input(shape=(max_length,))
        vae_loss, z1 = self._buildEncoder(x1, latent_rep_size, max_length, maskv)
    ...
    def _encoderMeanVar(self, x, latent_rep_size, max_length, epsilon_std = 0.01):
        h= GRU(max_length, return_sequences = True, name='encoder_gru_1')(x)
        h = Convolution1D(3, 3, activation = 'relu', name='conv_1')(h)
    ...
    def _buildEncoder(self, x, latent_rep_size, max_length, maskv=None, epsilon_std = 0.01):
        if maskv is None:
            h= GRU(max_length, return_sequences = True, name='encoder_gru_1')(x, mask=maskv)
        else:
            h= GRU(max_length, return_sequences = True, name='encoder_gru_1')(x)
        h = Convolution1D(3, 3, activation = 'relu', name='conv_1')(h)
        # h = Convolution1D(9, 9, activation = 'relu', name='conv_2')(h)
